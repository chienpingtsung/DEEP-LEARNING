...
parser.add_argument('--not_train_backbone', default=False, type=bool)
parser.add_argument('--not_train_seg', default=False, type=bool)
parser.add_argument('--not_train_edge', default=False, type=bool)
parser.add_argument('--not_train_merge', default=False, type=bool)
...
criterion = FocalLoss()
if args.not_train_backbone:
    for k, v in model.named_parameters():
        if k.startswith(('cont', 'maxpool', 'bottleneck')):
            v.requires_grad = False
if args.not_train_seg:
    for k, v in model.named_parameters():
        if k.startswith('seg_'):
            v.requires_grad = False
if args.not_train_edge:
    for k, v in model.named_parameters():
        if k.startswith('edge_'):
            v.requires_grad = False
if args.not_train_merge:
    for k, v in model.named_parameters():
...
for epoch in count(args.start_epoch):
    ...
    for image, label1, label2 in tq:
        ...
        loss = 0
        if not args.not_train_seg:
            loss += seg_loss
        if not args.not_train_edge:
            loss += edge_loss
        if not args.not_train_merge:
            loss += merge_loss
